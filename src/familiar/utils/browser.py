...
def create_llm(temperature: float = 0.5) -> Any:
    ...
    if provider == "openai":
        ...
        if not os.getenv("BROWSER_USE_API_KEY"):
            raise ValueError(
                "BROWSER_USE_API_KEY environment variable required for Browser Use provider"
            )
        ...
        if not os.getenv("AZURE_OPENAI_ENDPOINT") or not os.getenv("AZURE_OPENAI_API_KEY"):
            raise ValueError(
                "AZURE_OPENAI_ENDPOINT and AZURE_OPENAI_API_KEY environment variables required for Azure OpenAI provider"
            )
